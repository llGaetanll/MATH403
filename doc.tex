...
\definition {
  ...
  The set of Sylow $p$ subgroups of $G$ is denoted by $\Sylp(G)$.
}
...
\[
  \Sylp(G) = \{H\} \Leftrightarrow H \trianglelefteq G
\]

Having only a single Sylow $p$ subgroup means that it is normal!

{\bf Proof}: $\Sylp(G) = \{H\}$ if and only if $aHa^{-1} = H$ for all $a \in G$,
...
Let $|G| = p^e m$, $e \ge 1$, $p$ is prime, and $p$ does not divide $m$. Then
...
{\bf Theorem}: If $|G| = 35$, then $G \cong \Z_{15}$
...
\begin{proof}
  We know that $n_p = |\Sylp(G)|$ satisfies $n_p \mid q$ and $n_p \equiv 1 \mod
  q$. But $q \not\equiv 1 \mod p$, so $n_p = 1$. Additionally, $n_q$ satisfies $n_q
  \mid p$ and $n_q \equiv 1 \mod q$ so $n_q = 1$.

  {\bf Remember}: $n_p = 1$ if and only if $H \in \Sylp(G)$ is normal. So there
